function [ssdMat, normCorMat] = GetDepthMap(firstImage, secondImage, hw, dispRange)
...
ssdMat = zeros(imDimensions);
normCorMat = zeros(imDimensions);
...
for column = 1 : imDimensions(2)
   ...
   for row = 1 : imDimensions(1)
       ...
       ssdMat(row, column) = SSDcompare(leftWindow, rightWindows);
       normCorMat(row, column) = normCorCompare(leftWindow, rightWindows);     
   end
